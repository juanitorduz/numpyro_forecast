...
def test_forecaster_svi_shape_and_guide_not_resized(rng_key: Array) -> None:
    ...
    forecaster = Forecaster(model, data, empty_covariates(50), num_steps=60, rng_key=rng_key)
    ...
    fc10 = forecaster(data, empty_covariates(60), num_samples=32, rng_key=k1)
    fc5 = forecaster(data, empty_covariates(55), num_samples=16, rng_key=k2)
...
def test_forecaster_batch_size_matches_single_shot(rng_key: Array) -> None:
    ...
    forecaster = Forecaster(model, data, empty_covariates(40), num_steps=40, rng_key=rng_key)
    fc = forecaster(data, empty_covariates(46), num_samples=10, rng_key=rng_key, batch_size=3)
...
def test_forecaster_conditions_on_data(rng_key: Array) -> None:
    ...
    forecaster = Forecaster(model, data, empty_covariates(60), num_steps=600, rng_key=rng_key)
    fc = forecaster(data, empty_covariates(66), num_samples=200, rng_key=rng_key)
...
def test_forecaster_shape_both_backends(
    ...
    forecaster = forecaster_factory(
        RandomWalkModel(), train_data, empty_covariates(t), rng_key=key_fit
    )
    fc = forecaster(train_data, empty_covariates(56), num_samples=10, rng_key=key_forecast)
    assert fc.shape == (10, 6, 1)
...
def test_hmc_forecaster_shape(rng_key: Array) -> None:
    ...
    forecaster = HMCForecaster(
        model, data, empty_covariates(30), num_warmup=20, num_samples=20, rng_key=rng_key
    )
    fc = forecaster(data, empty_covariates(36), num_samples=10, rng_key=rng_key)
    assert fc.shape == (10, 6, 1)
    assert bool(jnp.all(jnp.isfinite(fc)))
...
def test_forecast_rejects_covariates_not_longer(rng_key: Array) -> None:
    model = RandomWalkModel()
    data = jnp.cumsum(0.1 * random.normal(rng_key, (30, 1)), axis=-2)
    forecaster = Forecaster(model, data, empty_covariates(30), num_steps=20, rng_key=rng_key)
    with pytest.raises(ValueError, match="covariates must extend beyond data"):
        forecaster(data, empty_covariates(30), num_samples=10, rng_key=rng_key)
...
def test_forecast_rejects_non_positive_num_samples(rng_key: Array) -> None:
    ...
    with pytest.raises(ValueError, match="num_samples must be positive"):
        forecaster(data, empty_covariates(36), num_samples=0, rng_key=rng_key)
...
def test_svi_forecaster_rejects_unequal_duration(rng_key: Array) -> None:
    ...
    with pytest.raises(ValueError, match="equal duration"):
        Forecaster(model, data, empty_covariates(25), num_steps=20, rng_key=rng_key)
...
def test_hmc_forecaster_rejects_unequal_duration(rng_key: Array) -> None:
    ...
    with pytest.raises(ValueError, match="equal duration"):
        HMCForecaster(
            model, data, empty_covariates(25), num_warmup=5, num_samples=5, rng_key=rng_key
        )
