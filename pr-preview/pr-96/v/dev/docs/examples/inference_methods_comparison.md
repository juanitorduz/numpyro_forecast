...
`SVI.run` fits `guide`, but a guide by itself is not yet a set of posterior samples: [draw_posterior](../../reference/functional.posterior.draw_posterior.md#numpyro_forecast.functional.posterior.draw_posterior) draws `2_000` samples of the latent sites from the fitted `guide`/`params` pair, in the same leading-sample-axis shape [to_datatree](../../reference/convert.to_datatree.md#numpyro_forecast.convert.to_datatree) and [forecast](../../reference/functional.prediction.forecast.md#numpyro_forecast.functional.prediction.forecast) expect.
