...
## Status after PR-A (updated 2026-08-27)

This section records what the implementation PRs decided on top of the proposal above; the sections above stay as written so the reasoning remains readable.

- **PR-A (#96) merged into `refactor3`**, together with #97 (multi-path Pathfinder), #98 (jax-first typing) and #99 (the final `device="host"` contract: CPU-device jax Arrays, NumPy only when no CPU backend is initialized, `jax<0.11.1` until pyro-ppl/numpyro#2245 ships). The 50,000-series `fresh_retail_stockout` notebook trains end to end on an A100 with `batch_size=250` / `device="host"`; that run is the acceptance test for the device machinery.
- **PR-B readback: nothing comes back.** `draw_posterior`, `forecast`, `predict_in_sample`, the closure-based `backtest`, and the posterior-first `to_datatree` stay as PR-A shaped them; `forecasting_model` does not return.
- **Compat rule.** "No breaking changes" means nothing #96 to #99 verified may regress (every device/batching knob keeps its name and semantics, the `Array | np.ndarray` host leaves stay CI-enforced). The renames and the layout move are hard cuts released together as 0.3.0 (bare semver tag: `release.yml` asserts `__version__ == github.ref_name`), with no deprecation aliases.
- **PR-C (#100): layout, renames, terminology.** A single `numpyro_forecast/models.py` (not a `models/` package, which would have needed re-exports and defeated `tests/test_docs_reference.py`'s `__module__` walk), `predictive.py` (posterior and prediction merged), `_offload.py` and `_validation.py` at the package root; `time_series` -> `innovations`, `markov_time_series` -> `markov_series` with a PyTree `xs`; one `predict(h, obs_dist, prediction)` dispatching on `isinstance(obs_dist, Distribution)` (the link form is the callable branch). `Horizon.from_data` stays the spelling: a module-level `horizon()` shadows the ubiquitous `horizon = 14` variable and its docs stem collides with `Horizon` on case-insensitive filesystems.
- **PR-D (#102): the `ssoe` block, final shape.** Smaller than proposed: `ssoe(h, name, y, init_carry, step, noise_dist, xs=None) -> SSOEResult(mu, mu_future, y_future)` with one callable `step(carry, x_t) -> (mu_t, carry_fn)` where `carry_fn(y_t, eps_t)` builds the next carry (the `Transition` shape of `markov_series`). No `obs=`/`obs_site`/`obs_mask` knobs and no `final_carry`: the block registers only `f"{name}_future"` under `plate("time_future", h.future, dim=-2)`; the caller writes the likelihood against `r.mu` (in-sample means) and `numpyro.deterministic("forecast", r.y_future)` when `h.future > 0`. `y` is a required argument and never `h.data` (the drivers call with `data=None`). The drawn errors must be exactly `(*batch, future, obs)` (a scalar noise with `obs > 1` would silently share one draw across series). Frozen gates go through an `xs` leaf padded with `arrays.pad_future(gate, h.future)` and are read from `x_t`, never `y_t`. `SSOEResult` is a frozen dataclass like every other result type in the package. All six hand-rolled notebooks map onto this shape; the two-channel Croston composition lives in `tests/example_models.py`.
- **PR-E (#103, #104, #105): all thirteen notebooks on the API.** E1 `censored_demand`/`exponential_smoothing_state_space`/`arma`, E2 `croston`/`tsb`/`availability_tsb`, E3 `electricity_forecast`/`electricity_forecast_calibration`/`hierarchical_forecasting_1`/`hierarchical_forecasting_2`; ty checks every notebook with no excludes; no notebook needed a `horizon()` spelling, so the PR-C decision to keep `Horizon.from_data` stands; models with parameters are `make_*_model(...)` factories returning a model function; the `ssoe` recipe (likelihood against `r.mu`, `deterministic("forecast", r.y_future)`) is used verbatim in the six hand-rolled notebooks.
- **PR-F (#106): README and docs.** The README presents the package as a conceptual port of `pyro.contrib.forecast` with a functional flavor (JAX/NumPyro are functional; Pyro's ideas, not its class hierarchy), links PyMC-Forecast as the related class-based project, and its examples are native pytest doctests (`--doctest-glob=README.md`, `README.md` in `testpaths`), so the landing page is executed by the suite. The canonical spelling is `from numpyro_forecast import ...`; the `nf` alias proposed above was not adopted. The API reference sections were reordered (building blocks first) and matplotlib moved to the `dev` extra.
- **Dependency slimming (item 7): arviz stays a core dependency** by decision (2026-08-26); only matplotlib left the hard dependencies, inside PR-F on `refactor3` rather than in a PR-G targeting `main`; there is no `[arviz]` extra, and the CI base-import leak list and `tests/test_package.py` were not extended with `arviz`/`xarray`/`pandas` as a consequence.
- **Run-configuration guard: option (a).** With `fit_mcmc` gone, the BlackJAX kernels document `chain_method="sequential"` and `num_warmup=0`; `KernelConfigError` fires only from `kernel.init` for a kernel constructed without a model.
- **Release mechanics.** The tag is bare `0.3.0` (the `v0.3.0` spelling used above and in earlier versions of this section was wrong); `refactor3` is dropped from the workflow triggers in a follow-up PR on `main` after the release, not in the integration PR (a PR into `refactor3` whose merged workflow file lists only `main` would get no CI). The GPU re-execution of `fresh_retail_stockout` is not a release gate (decision 2026-08-27); the device contract is CI-enforced by `tests/test_host_pipeline.py`.
- **Remaining:** the release PR into `refactor3` (version `0.3.0`, `.test_durations` refresh), the integration merge #101, and the `0.3.0` release; this PR stays open as the design record.
