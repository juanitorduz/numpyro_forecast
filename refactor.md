...
## Status after PR-A (updated 2026-08-26)
...
- **Compat rule.** "No breaking changes" means nothing #96 to #99 verified may regress (every device/batching knob keeps its name and semantics, the `Array | np.ndarray` host leaves stay CI-enforced). The renames and the layout move are hard cuts released together as v0.3.0, with no deprecation aliases.
...
- **Remaining:** PR-E (the ten notebooks, three PRs by cost), PR-F (README and docs restructure), PR-G (dependency slimming, targeting `refactor3` rather than `main` because `convert.py` was rewritten on the branch), then the integration merge and v0.3.0 in one sitting.
