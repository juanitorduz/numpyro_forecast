...
Dimensions beyond `(time, obs)` stack leftward, and the two loop-shaped blocks take opposite approaches to plates. `innovations` is called inside the plates you open yourself: the [multi-series example](https://juanitorduz.github.io/numpyro_forecast/examples/hierarchical_forecasting_1.html) wraps it in `plate("n_series", n_series, dim=-1)`, and the [hierarchical origin-destination example](https://juanitorduz.github.io/numpyro_forecast/examples/hierarchical_forecasting_2.html) opens `origin` at `dim=-3` and `destin` at `dim=-1` and puts the call inside the latter. `markov_series` flips the idiom: it rejects an enclosing plate and takes `plates=[(name, size)]`, which it opens inside the scan body, the only placement NumPyro supports for scan plus plate.
